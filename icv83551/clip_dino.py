...
def get_similarity_no_loop(text_features, image_features):
    ...
    ############################################################################
    text_norm = torch.nn.functional.normalize(text_features.float(), dim=-1)
    image_norm = torch.nn.functional.normalize(image_features.float(), dim=-1)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    processed = [clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
    images_tensor = torch.cat(processed, dim=0).to(device)
    ...
    pred_classes = [class_texts[idx] for idx in best_indices]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        self.image_features = torch.nn.functional.normalize(self.image_features.float(), dim=-1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=5e-3, weight_decay=5e-2)
